...
/**
 * Firma real "Raúl" (public/images/firma-raul.png) revelada con efecto Apple Hello.
 * Los paths son la guía del trazo; el PNG se muestra a través de una máscara animada.
 */
type RaulProps = ComponentProps<typeof motion.svg> & {
  speed?: number;
  onAnimationComplete?: () => void;
  /** 0→1 ligado al scroll; si existe, dibuja mientras bajas */
  scrollProgress?: MotionValue<number>;
  /** lime = amarillo; red = firma roja encima; white = trazo blanco vectorial */
  tone?: "lime" | "white" | "red";
};

/** Centerlines over viewBox matching firma-raul.png (1024×775) */
const RAUL_SEGMENTS = [
  // R — tall loop
  "M210 540C185 420 195 260 255 155C310 65 445 45 545 115C620 170 635 275 575 355C520 425 415 455 320 420C255 390 230 330 265 285C310 230 410 235 495 295",
  // R — diagonal leg
  "M330 300C390 355 455 420 530 500",
  // a
  "M515 360C530 320 595 305 635 345C665 380 650 440 600 450C555 460 520 420 525 375",
  // u
  "M640 390C650 445 700 465 745 430C770 405 790 420 805 445C812 420 818 370 822 320",
  // l
  "M822 320C830 250 845 175 860 130C875 95 905 120 895 185C885 260 890 360 905 455",
  // underline
  "M250 415C380 430 560 445 740 455C820 460 900 455 955 440",
  // accent (ú)
  "M720 275C735 260 755 245 775 235",
] as const;
...
const AUTO_TIMINGS = [
  ...
  { delay: 1.0, duration: 0.45 },
  { delay: 1.35, duration: 0.4 },
  { delay: 1.65, duration: 0.5 },
  { delay: 1.9, duration: 0.7 },
  { delay: 2.45, duration: 0.35 },
];

function ScrollVisibleStroke({
  d,
  range,
  scrollProgress,
  stroke,
  ...
  const pathLength = useTransform(scrollProgress, range, [0, 1]);
  ...
  return (
    <motion.path
      ...
      transition={{
        ...
        ease: "easeInOut",
        delay: calc(delay),
        opacity: { duration: calc(0.2), delay: calc(delay) },
      }}
      onAnimationComplete={onComplete}
    />
  );
...
function AppleRaulEffect({
  ...
  const maskId = `raul-draw-mask-${uid}`;

  const visibleStroke = tone === "white" ? "#ffffff" : "#f2f2ea";
  const visibleStrokeWidth = tone === "white" ? 22 : 20;

  const renderStrokes = (mode: "mask" | "visible") =>
    RAUL_SEGMENTS.map((d, i) => {
      const onComplete =
        i === RAUL_SEGMENTS.length - 1 ? onAnimationComplete : undefined;
      ...
      if (mode === "visible") {
        return (
          <AutoVisibleStroke
            ...
            d={d}
            speed={speed}
            delay={AUTO_TIMINGS[i].delay}
            duration={AUTO_TIMINGS[i].duration}
            stroke={visibleStroke}
            strokeWidth={visibleStrokeWidth}
            onComplete={onComplete}
          />
  ...
  if (tone === "white") {
    return (
      ...
      >
        <title>Raúl</title>
        {renderStrokes("visible")}
      </motion.svg>
    );
  }

  const pngClass =
    tone === "red" ? "raul-signature-png-red" : undefined;
  const pngStyle =
    tone === "lime" ? ({ mixBlendMode: "screen" } as const) : undefined;
    ...
    >
      ...
      <defs>
        <mask
          ...
          height="775"
        >
          <rect width="1024" height="775" fill="black" />
          {renderStrokes("mask")}
        </mask>
      </defs>

      <image
        href="/images/firma-raul.png"
        width="1024"
        height="775"
        mask={`url(#${maskId})`}
        className={pngClass}
