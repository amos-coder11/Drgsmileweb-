/* WebGL fluid cursor — adapted from Inspira UI (MIT) */
...
export function initFluidCursor(
 ...
 function onMouseMove(e: MouseEvent) {
  ...
  function onTouchStart(e: TouchEvent) {
    ...
    for (let i = 0; i < touches.length; i++) {
      const { x, y } = toLocal(touches[i].clientX, touches[i].clientY);
 ...
 function generateColor(): ColorRGB {
    const base = 0.28 + Math.random() * 0.14;
    return { r: base * 0.96, g: base, b: base * 1.04 };
  }
